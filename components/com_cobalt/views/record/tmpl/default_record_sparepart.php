...
//JImport('b0.Sparepart.Sparepart');
...
echo JLayoutHelper::render('b0.openGraph', [
    'og' => setOpenGraph($item,
        $item->fields_by_id[SparepartIds::ID_IMAGE] ?? null,
        $item->fields_by_id[SparepartIds::ID_GALLERY] ??  null,
        $item->fields_by_id[SparepartIds::ID_VIDEO] ?? null),
    'doc' => $this->document,
]);

/** @var boolean $is_special */
$is_special = ($item->fields_by_id[SparepartIds::ID_IS_SPECIAL]->value == 1) ? true : false;

/** @var boolean $is_by_order */
$is_by_order = ($item->fields_by_id[SparepartIds::ID_IS_BY_ORDER]->value == 1) ? true : false;

/** @var boolean $is_general */
$is_general = (!$is_special && !$is_by_order) ? true : false;

/** @var boolean $is_original */
$is_original = $item->fields_by_id[SparepartIds::ID_IS_ORIGINAL]->value == 1 ? true : false;

$prices = [
    'general'=> $item->fields_by_id[SparepartIds::ID_PRICE_GENERAL]->value,
    'special'=> isset($item->fields_by_id[SparepartIds::ID_PRICE_SPECIAL]) ? ($item->fields_by_id[SparepartIds::ID_PRICE_SPECIAL]->value) : 0,
    'simple'=> $item->fields_by_id[SparepartIds::ID_PRICE_SIMPLE]->value,
    'silver'=> $item->fields_by_id[SparepartIds::ID_PRICE_SILVER]->value,
    'gold'=> $item->fields_by_id[SparepartIds::ID_PRICE_GOLD]->value,
    'delivery'=> $item->fields_by_id[SparepartIds::ID_PRICE_DELIVERY]->value
];

$this->document->setTitle($item->title.' купить в '. $siteName . ' с бесплатной доставкой по СПб');
...
<div <?= $microdata::htmlProperty('Product');?> <?= $microdata::htmlScope('Product')?> >
    ...
    <?= $microdata::htmlMeta(JUri::base() . $item->fields_by_id[SparepartIds::ID_IMAGE]->value['image'], 'image');?>
	...
	<?php if (isset($item->fields_by_id[SparepartIds::ID_MANUFACTURER])){
		echo $microdata::htmlMeta($item->fields_by_id[SparepartIds::ID_MANUFACTURER]->result, 'manufacturer');
	}?>
	<?php if (isset($item->fields_by_id[SparepartIds::ID_ORIGINAL_CODE])){
		echo $microdata::htmlMeta($item->fields_by_id[SparepartIds::ID_ORIGINAL_CODE]->raw, 'mpn');
	}?>
	<?php if (isset($item->fields_by_id[SparepartIds::ID_PRODUCT_CODE])){
		echo $microdata::htmlMeta($item->fields_by_id[SparepartIds::ID_PRODUCT_CODE]->raw, 'sku');
	}?>
...
<div class="uk-navbar">
    <div class="uk-navbar-nav">
        <?php if (isset($item->fields_by_id[SparepartIds::ID_SUBTITLE])) :?>
            <p class="uk-article-lead"><?= $item->fields_by_id[SparepartIds::ID_SUBTITLE]->result;?></p>
        <?php endif; ?>
    </div>
    <div class="uk-navbar-flip">
        <?php if (isset($item->fields_by_id[SparepartIds::ID_PRODUCT_CODE])) :?>
            <p><strong>Код товара: <?= $item->fields_by_id[SparepartIds::ID_PRODUCT_CODE]->result;?></strong></p>
        <?php endif;?>
    </div>
</div>
...
<div class="uk-grid" data-uk-grid-match>
    <div class="uk-width-medium-2-5 uk-text-center"> <!-- Картинка -->
        <?php if (isset($item->fields_by_id[SparepartIds::ID_IMAGE])) echo $item->fields_by_id[SparepartIds::ID_IMAGE]->result;?>
    </div>
...
<div class="uk-grid" data-uk-grid-match>
    <div class="uk-width-medium-2-5">
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
	        <?php if (isset($item->fields_by_id[SparepartIds::ID_MODEL])){
		        render_field($item->fields_by_id[SparepartIds::ID_MODEL], $user);
	        }?>
	        <?php if (isset($item->fields_by_id[SparepartIds::ID_MOTOR])){
		        render_field($item->fields_by_id[SparepartIds::ID_MOTOR], $user);
	        }?>
	        <?php if (isset($item->fields_by_id[SparepartIds::ID_MANUFACTURER])){
                render_field($item->fields_by_id[SparepartIds::ID_MANUFACTURER], $user);
            }?>
            <?php if (isset($item->fields_by_id[SparepartIds::ID_VENDOR_CODE])){
                render_field($item->fields_by_id[SparepartIds::ID_VENDOR_CODE], $user);
            }?>
            <?php if (isset($item->fields_by_id[SparepartIds::ID_ORIGINAL_CODE])){
                render_field($item->fields_by_id[SparepartIds::ID_ORIGINAL_CODE], $user, 'mpn');
            }?>
        </div>
    </div>
    <div class="uk-width-medium-3-5">
        <div class="uk-panel uk-panel-box uk-panel-box-secondary">
           <!-- Блок цен по дисконтным картам -->
            ...
            <?php } ?>
            <!-- Наличие Седова -->
            <?php if (isset($item->fields_by_id[SparepartIds::ID_SEDOVA])) {
                render_field($item->fields_by_id[SparepartIds::ID_SEDOVA], $user);
            }?>
            <!-- Наличие Химиков -->
            <?php if (isset($item->fields_by_id[SparepartIds::ID_KHIMIKOV])) {
                render_field($item->fields_by_id[SparepartIds::ID_KHIMIKOV], $user);
            }?>
            <!-- Наличие Жукова -->
            <?php if (isset($item->fields_by_id[SparepartIds::ID_ZHUKOVA])) {
                render_field($item->fields_by_id[SparepartIds::ID_ZHUKOVA], $user);
            }?>
            <!-- Наличие Культуры -->
            <?php if (isset($item->fields_by_id[SparepartIds::ID_KULTURY])) {
                render_field($item->fields_by_id[SparepartIds::ID_KULTURY], $user);
            }?>
        </div>
    </div>
</div>
...
<!-- Описание -->
<div>
    <?php if (isset($item->fields_by_id[SparepartIds::ID_DESCRIPTION])) echo $item->fields_by_id[SparepartIds::ID_DESCRIPTION]->result; ?>
</div>
<!-- Галерея -->
<?php if (isset($item->fields_by_id[SparepartIds::ID_GALLERY])) {
    echo '<hr class="uk-article-divider">';
    echo $item->fields_by_id[SparepartIds::ID_GALLERY]->result;
} ?>
<!-- Видео -->
<?php if (isset($item->fields_by_id[SparepartIds::ID_VIDEO])) {
    echo '<hr class="uk-article-divider">';
    echo $item->fields_by_id[SparepartIds::ID_VIDEO]->result;
} ?>
<!-- Статистика -->
...
<div class="uk-margin uk-width-medium-1-1">
    ...
    <ul class="uk-tab" data-uk-tab="{connect:'#tab-content'}">
        <?php
             if (isset($item->fields_by_id[SparepartIds::ID_CHARACTERISTICS])) {tab_title("Характеристики", 1);}
             if (isset($item->fields_by_id[SparepartIds::ID_ANALOGS])) {tab_title("Аналоги", 0);}
             if (isset($item->fields_by_id[SparepartIds::ID_ASSOCIATED])) {tab_title("Сопутствующие", 0);}
             if (isset($item->fields_by_id[SparepartIds::ID_WORKS])) {tab_title("Автосервис", 0);}
             if (isset($item->fields_by_id[SparepartIds::ID_FOR_READING])) {tab_title("Почитать", 0);}
        ?>
    </ul>
    <!-- Контент закладок -->
    <ul class="uk-switcher uk-margin" id="tab-content">
        <?php
            if (isset($item->fields_by_id[SparepartIds::ID_CHARACTERISTICS])) {tab_content($item->fields_by_id[SparepartIds::ID_CHARACTERISTICS]->result);}
            if (isset($item->fields_by_id[SparepartIds::ID_ANALOGS])) {tab_content($item->fields_by_id[SparepartIds::ID_ANALOGS]->result);}
            if (isset($item->fields_by_id[SparepartIds::ID_ASSOCIATED])) {tab_content($item->fields_by_id[SparepartIds::ID_ASSOCIATED]->result);}
            if (isset($item->fields_by_id[SparepartIds::ID_WORKS])) {tab_content($item->fields_by_id[SparepartIds::ID_WORKS]->result);}
            if (isset($item->fields_by_id[SparepartIds::ID_FOR_READING])) {tab_content($item->fields_by_id[SparepartIds::ID_FOR_READING]->result);}
        ?>
    </ul>
</div>
<!-- Конец закладок -->
...
<!--noindex-->
    ...
    <div class="uk-margin-large-top uk-margin-large-bottom" itemprop="description">
